...
class LaboratoireController extends AbstractController
{
    ...
    #[Route('/map', name: 'app_laboratoire_map')]
public function map(EntityManagerInterface $em): Response
{
    $markers = [];
    $laboratoires = $em->getRepository(Laboratoire::class)->findAll();
    
    foreach ($laboratoires as $laboratoire) {
        $markers[] = [
            'lat' => $laboratoire->getLatitude(),
            'lng' => $laboratoire->getLongitude(),
            'popup' => $laboratoire->getNom(),
        ];
    }
    
    return $this->render('laboratoire/map.html.twig', [
        'markers' => $markers,
    ]);
}
